...
class WorldMap(object):
    ...
    def obstacle_edge_at(self, x, y, distance):
        ...
        for edge in self.edges:
			...
			current_distance, is_inside_edge = get_distance(x,y,edge)		
	...
	def get_distance(self, x, y, edge):
		...
		return q_dot_n - d, min(edge[0][0], edge[1][0]) < q_prime[0] < max(edge[0][0], edge[1][0]) 
			            and min(edge[0][1], edge[1][1]) < q_prime[1] < max(edge[0][1], edge[1][1])
